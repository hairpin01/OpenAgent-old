...
from OpenAgentLib.PluginSDK import CapabilityCallContext, CapabilityClient, CapabilityFamily  # noqa: E402
...
class BrokerTransport:
    def __init__(self, broker: CapabilityBroker, call: ToolCall, policy_request: ToolPolicyRequest, grant: CapabilityGrant) -> None:
        self.broker = broker
        self.call = call
    ...
    def request(self, frame: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        return self.broker.dispatch(self.call, self.policy_request, self.grant, request).to_envelope()
...
class AstGrepBackend:
    ...
    def invoke(self, operation: str, payload: Mapping[str, Any], grant: CapabilityGrant) -> Mapping[str, Any]:
        ...
        target.write_text(target.read_text(encoding="utf-8").replace("print", "logger.info"), encoding="utf-8")
        return {"exit_code": 0, "stdout": "--- fixture.py\n+++ fixture.py\n", "stderr": ""}


def _modules() -> tuple[object, ...]:
    return tuple(importlib.import_module(f"plugins.{module}") for module in TARGET_MODULES)
...
def _capability(call: ToolCall, transport: RecordingTransport | BrokerTransport, *, actor_scope: str = "actor:test") -> CapabilityClient:
    return CapabilityClient(CapabilityCallContext("host-1", call.call_id, call.spec.canonical_id, actor_scope, "grant-1"), transport)
...
def _policy(call: ToolCall) -> tuple[ToolPolicyEngine, ToolPolicyRequest]:
    ...
    confirmation = ConfirmationState.APPROVED if call.spec.confirmation.value == "required" else ConfirmationState.MISSING
    request = ToolPolicyRequest(
        frozenset({call.spec.canonical_id}),
        call.spec.capabilities,
        confirmation=confirmation,
        confirmation_grant=ToolConfirmationGrant.for_call("confirmation-1", call) if confirmation is ConfirmationState.APPROVED else None,
...
def _request(grant: CapabilityGrant, capability: CapabilityFamily, operation: str, payload: Mapping[str, Any]) -> CapabilityRequest:
...
def test_matrix_canonical_ids_and_aliases_are_declared_exactly() -> None:
    expected = {entry.canonical_id: entry for entry in TOOL_COMPATIBILITY_MATRIX if entry.source_module in TARGET_MODULES}
    declared = {tool.canonical_id: tool for module in _modules() for tool in module.MANIFEST.tools}
...
def test_resource_modules_have_no_ambient_or_direct_resource_apis() -> None:
    ...
    forbidden = (
        "create_subprocess_shell", "create_subprocess_exec", "shell=True", "os.system", "subprocess",
        "aiohttp", "socket", "Path.cwd", "pathlib", "self.agent", "self.config", "tool_registry", "tool_map",
    )
...
def test_terminal_argv_is_allowlisted_and_shell_metacharacters_remain_data() -> None:
    ...
    call = _call(terminal, "terminal.run", {"argv": ["echo", "; touch /tmp/pwned"], "cwd": "."})
    transport = RecordingTransport({"ok": True, "data": {"exit_code": 0, "stdout": "; touch /tmp/pwned", "stderr": ""}})
    ...
    with pytest.raises(ValueError, match="allowlisted"):
        terminal.HANDLERS["terminal.run"](blocked, _capability(blocked, RecordingTransport()))
    find_escape = _call(terminal, "terminal.run", {"argv": ["find", ".", "-exec", "/bin/sh", "-c", "id", ";"], "cwd": "."})
    with pytest.raises(ValueError, match="allowlisted"):
        terminal.HANDLERS["terminal.run"](find_escape, _capability(find_escape, RecordingTransport()))
    path_escape = _call(terminal, "terminal.run", {"argv": ["cat", "/etc/passwd"], "cwd": "."})
    with pytest.raises(ValueError, match="grant-relative"):
        terminal.HANDLERS["terminal.run"](path_escape, _capability(path_escape, RecordingTransport()))


def test_terminal_process_output_is_bounded() -> None:
    terminal = importlib.import_module("plugins.terminal")
    call = _call(terminal, "terminal.run", {"argv": ["echo", "large"], "cwd": "."})
    transport = RecordingTransport({"ok": True, "data": {"exit_code": 0, "stdout": "x" * 20_000, "stderr": ""}})
...
def test_resource_handlers_reject_malformed_capability_success_frames() -> None:
    ...
    with pytest.raises(ValueError, match="boolean ok"):
        terminal.HANDLERS["terminal.run"](call, _capability(call, RecordingTransport({"data": {}})))
    with pytest.raises(ValueError, match="bounded text"):
        terminal.HANDLERS["terminal.run"](
            call,
            _capability(call, RecordingTransport({"ok": True, "data": {"exit_code": 0, "stdout": "ok"}})),
        )


def test_terminal_inspect_uses_only_fixed_metadata_argv() -> None:
    terminal = importlib.import_module("plugins.terminal")
    call = _call(terminal, "terminal.inspect", {"operation": "git-status", "cwd": "project"})
    transport = RecordingTransport({"ok": True, "data": {"exit_code": 0, "stdout": " M file.py", "stderr": ""}})
...
def test_scoped_ast_grep_replace_changes_only_granted_fixture(tmp_path: Path) -> None:
    ...
    call = _call(ast_grep, "ast_grep.replace", {
        "pattern": "print($$$)", "rewrite": "logger.info($$$)", "lang": "python", "path": "fixture.py", "apply": True,
    })
    ...
    grant = CapabilityGrant.for_call(
        ...
        frozenset({"run"}),
        {
            "executables": ["ast-grep"], "cwd_root": str(tmp_path), "max_timeout_seconds": 30,
            "max_output_bytes": 24_000, "max_args": 128, "max_arg_length": 8_192, "env_allowlist": [],
        },
    )

    result = ast_grep.HANDLERS["ast_grep.replace"](
        call,
        _capability(call, BrokerTransport(broker, call, policy_request, grant), actor_scope=grant.actor_scope),
    )
    ...
    assert result == {"ok": True, "exit_code": 0, "result": "--- fixture.py\n+++ fixture.py\n", "truncated": False, "applied": True}


def test_ast_grep_rejects_traversal_before_capability_request() -> None:
    ast_grep = importlib.import_module("plugins.ast_grep")
    call = _call(ast_grep, "ast_grep.search", {"pattern": "print($$$)", "lang": "python", "path": "../secret.py"})
...
def test_ast_grep_rejects_glob_traversal_before_capability_request() -> None:
    ast_grep = importlib.import_module("plugins.ast_grep")
    call = _call(ast_grep, "ast_grep.search", {
        "pattern": "print($$$)", "lang": "python", "path": "src", "globs": ["../**/*.py"],
    })
    transport = RecordingTransport()
...
def test_granted_workspace_list_read_and_https_fetch_use_narrow_operations() -> None:
    ...
    list_transport = RecordingTransport({"ok": True, "data": {"entries": ["README.md", "src/"]}})
    read_transport = RecordingTransport({"ok": True, "data": {"content": "hello"}})
    fetch_transport = RecordingTransport({
        "ok": True,
        "data": {"url": "https://example.com", "content_type": "text/plain", "content": "hello"},
    })

    assert terminal.HANDLERS["terminal.list_files"](list_call, _capability(list_call, list_transport))["entries"] == ["README.md", "src/"]
    assert terminal.HANDLERS["terminal.read_file"](read_call, _capability(read_call, read_transport))["content"] == "hello"
    assert web.HANDLERS["web.fetch_url"](fetch_call, _capability(fetch_call, fetch_transport))["content"] == "hello"
...
def test_file_edit_uses_read_version_as_atomic_write_guard() -> None:
    ...
    call = _call(file_plugin, "file.edit", {"path": "notes/todo.txt", "search": "old", "replace": "new"})
    transport = RecordingTransport(
    ...
    assert result == {"ok": True, "path": "notes/todo.txt", "changed": True, "version": "hash-2"}
    assert transport.frames[1]["payload"] == {
        "path": "notes/todo.txt", "content": "new\n", "mode": "overwrite", "expected_hash": "hash-1",
    }
...
def test_file_empty_write_uses_backend_changed_result() -> None:
    ...
    transport = RecordingTransport({"ok": True, "data": {"version": "hash-2", "changed": True}})

    assert file_plugin.HANDLERS["file.write"](call, _capability(call, transport))["changed"] is True
...
def test_file_patch_rejects_escaped_header_before_reading() -> None:
    file_plugin = importlib.import_module("plugins.file")
    call = _call(file_plugin, "file.patch", {
        "path": "notes/todo.txt",
        "patch": "--- a/../secret.txt\n+++ b/../secret.txt\n@@ -1 +1 @@\n-old\n+new\n",
    })
    transport = RecordingTransport()
...
def test_broker_rejects_symlink_escape_and_normalizes_guarded_write(tmp_path: Path) -> None:
    file_plugin = importlib.import_module("plugins.file")
    call = _call(file_plugin, "file.write", {"path": "notes/todo.txt", "content": "new"})
    policy, policy_request = _policy(call)
    outside = tmp_path.parent / "outside"
    outside.mkdir(exist_ok=True)
    (tmp_path / "link").symlink_to(outside, target_is_directory=True)
    grant = CapabilityGrant.for_call("grant-1", "host-1", call, CapabilityFamily.WORKSPACE_FS, frozenset({"write"}), {"root": str(tmp_path)})
    backend = RecordingFilesystemBackend()
    broker = CapabilityBroker(policy, {CapabilityFamily.WORKSPACE_FS: backend})

    escaped = broker.dispatch(call, policy_request, grant, _request(grant, CapabilityFamily.WORKSPACE_FS, "write", {"path": "link/escape.txt", "content": "x"}))
    accepted = broker.dispatch(call, policy_request, grant, _request(grant, CapabilityFamily.WORKSPACE_FS, "write", {
        "path": "notes/todo.txt", "content": "new", "mode": "overwrite", "expected_hash": "hash-1",
    }))
    ...
    assert backend.calls == [{
        "operation": "write",
        "payload": {"path": str(tmp_path / "notes" / "todo.txt"), "content": "new", "mode": "overwrite", "expected_hash": "hash-1"},
    }]
...
def test_https_capability_blocks_private_dns_and_private_redirects() -> None:
    ...
    policy, policy_request = _policy(call)
    grant = CapabilityGrant.for_call(
        "grant-1", "host-1", call, CapabilityFamily.HTTPS_FETCH, frozenset({"fetch"}),
        {"max_timeout_seconds": 20, "max_bytes": 262_144},
    )
    request = _request(grant, CapabilityFamily.HTTPS_FETCH, "fetch", {
        "url": "https://initial.invalid", "timeout_seconds": 10, "max_bytes": 1_024,
    })
    private_broker = CapabilityBroker(policy, {CapabilityFamily.HTTPS_FETCH: RecordingHttpsBackend()}, resolver=lambda *_args, **_kwargs: [(None, None, None, None, ("127.0.0.1", 443))])
    redirect_broker = CapabilityBroker(policy, {CapabilityFamily.HTTPS_FETCH: RecordingHttpsBackend(["https://redirect.invalid"])}, resolver=_global_dns)

    assert private_broker.dispatch(call, policy_request, grant, request).error is CapabilityErrorCode.INVALID_REQUEST
    assert redirect_broker.dispatch(call, policy_request, grant, request).error is CapabilityErrorCode.BACKEND_ERROR
...
def test_broker_rejects_undeclared_workspace_operation() -> None:
    ...
    policy, policy_request = _policy(call)
    grant = CapabilityGrant.for_call("grant-1", "host-1", call, CapabilityFamily.WORKSPACE_FS, frozenset({"read"}), {"root": str(ROOT)})
    broker = CapabilityBroker(policy, {CapabilityFamily.WORKSPACE_FS: RecordingFilesystemBackend()})

    response = broker.dispatch(call, policy_request, grant, _request(grant, CapabilityFamily.WORKSPACE_FS, "delete", {"path": "notes/todo.txt"}))
    wrong_capability = broker.dispatch(call, policy_request, grant, _request(grant, CapabilityFamily.PROCESS, "run", {
        "argv": ["echo", "no"], "cwd": ".", "timeout_seconds": 1, "max_output_bytes": 1,
    }))
