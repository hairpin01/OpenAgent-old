    ...
    if isinstance(value, Mapping):
        return (
            ...
            if all(isinstance(key, str) for key in value)
                ...
                CapabilityProtocolError("JSON keys must be strings")
            )
        )
...
class CapabilityGrant:
    ...
    def __post_init__(self) -> None:
        ...
        operations = frozenset(
            _required(operation, "operation").lower() for operation in self.operations
        )
        if not operations:
...
class CapabilityRequest:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "capability", CapabilityFamily(self.capability))
        object.__setattr__(
            self, "operation", _required(self.operation, "operation").lower()
        )
    ...
    def from_envelope(cls, value: Any) -> "CapabilityRequest":
        ...
        if (
            ...
            or value.get("kind") != "capability-request"
        ):
            raise CapabilityProtocolError(
                "capability request has unknown or missing fields"
            )
...
class CapabilityResponse:
    ...
    def __post_init__(self) -> None:
        ...
        if self.ok == (self.error is not None):
            raise CapabilityProtocolError(
                "responses require exactly success data or an error"
            )
...
def _resolve_workspace_path(root: str | Path, relative: str) -> Path:
    try:
        ...
        raise CapabilityProtocolError(
            "filesystem path escapes its granted root"
        ) from exc
...
class CapabilityBroker:
    ...
    def __init__(
        ...
        backends: Mapping[CapabilityFamily, CapabilityBackend],
        *,
    ...
    def dispatch(
        ...
        if (
            self._policy.evaluate(call, policy_request).kind
            is not PolicyDecisionKind.ALLOW
        ):
    ...
    @staticmethod
    def _grant_matches(
        call: ToolCall, grant: CapabilityGrant, request: CapabilityRequest
    ) -> bool:
        return (
            grant.grant_id == request.grant_id
            and grant.host_request_id == request.host_request_id
            and grant.call_id == call.call_id == request.call_id
            and grant.canonical_tool_id
            == call.spec.canonical_id
            == request.canonical_tool_id
...
_TELEGRAM_OPERATION_FIELDS = MappingProxyType(
    {
        ...
        "chat-info": frozenset({"peer_id"}),
        "chat-participants": frozenset({"peer_id", "limit"}),
        "chat-admins": frozenset({"peer_id"}),
        "chat-permissions": frozenset({"peer_id"}),
        "chat-common-with-user": frozenset({"user_id", "limit"}),
        "chat-set-title": frozenset({"peer_id", "title"}),
        "chat-set-about": frozenset({"peer_id", "about"}),
        "chat-set-username": frozenset({"peer_id", "username"}),
        "chat-slowmode": frozenset({"peer_id", "seconds"}),
        "chat-invite-link": frozenset({"peer_id"}),
        "contacts-add": frozenset({"user_id", "first_name", "last_name", "phone"}),
        "contacts-delete": frozenset({"user_id"}),
        "contacts-block": frozenset({"user_id"}),
        "contacts-unblock": frozenset({"user_id"}),
        "contacts-entity": frozenset({"user_id"}),
        "creation-channel": frozenset({"title", "about"}),
        "creation-group": frozenset({"title", "about"}),
        "creation-bot": frozenset({"name", "username", "about"}),
        "creation-private-invite": frozenset({"invite_id"}),
        "dialog-list-private": frozenset({"limit"}),
        "dialog-list-groups": frozenset({"limit"}),
        "dialog-list-all": frozenset({"limit"}),
        "dialog-search": frozenset({"query", "limit"}),
        "dialog-archive": frozenset({"peer_id"}),
        "dialog-unarchive": frozenset({"peer_id"}),
        "dialog-leave": frozenset({"peer_id"}),
        "dialog-export-invite": frozenset({"peer_id"}),
        "dialog-get-photo": frozenset({"peer_id"}),
        "dialog-set-photo": frozenset({"peer_id", "media_id"}),
        "message-reply": frozenset({"peer_id", "message_id", "text"}),
        "message-forward": frozenset({"peer_id", "message_id", "destination_peer_id"}),
        "message-pin": frozenset({"peer_id", "message_id"}),
        "message-search": frozenset({"peer_id", "query", "limit"}),
        "message-history": frozenset({"peer_id", "limit"}),
        "message-mark-read": frozenset({"peer_id", "message_id"}),
        "message-typing": frozenset({"peer_id"}),
        "message-schedule": frozenset({"peer_id", "text", "schedule_at"}),
        "message-draft": frozenset({"peer_id", "text"}),
        "moderation-mute": frozenset({"peer_id", "user_id", "until_seconds"}),
        "moderation-unmute": frozenset({"peer_id", "user_id"}),
        "moderation-ban": frozenset({"peer_id", "user_id", "reason"}),
        "moderation-unban": frozenset({"peer_id", "user_id"}),
        "moderation-kick": frozenset({"peer_id", "user_id"}),
        "moderation-promote": frozenset({"peer_id", "user_id", "rights"}),
        "moderation-demote": frozenset({"peer_id", "user_id"}),
        "moderation-pin": frozenset({"peer_id", "message_id"}),
        "moderation-delete-messages": frozenset({"peer_id", "message_id"}),
        "moderation-get-admins": frozenset({"peer_id"}),
        "profile-get": frozenset({"user_id"}),
        "profile-get-full": frozenset({"user_id"}),
        "profile-get-me": frozenset(),
        "profile-update-name": frozenset({"first_name", "last_name"}),
        "profile-update-bio": frozenset({"bio"}),
        "profile-update-username": frozenset({"username"}),
        "profile-set-photo": frozenset({"media_id"}),
        "profile-download-photo": frozenset({"user_id"}),
        "profile-get-photos": frozenset({"user_id", "limit"}),
        "profile-common-chats": frozenset({"user_id", "limit"}),
        "send-media": frozenset({"peer_id", "media_id", "caption"}),
...
_TELEGRAM_REQUIRED_FIELDS = MappingProxyType(
    {
        "get-message": frozenset({"message_id"}),
        "send-message": frozenset({"peer_id", "text"}),
        "edit-message": frozenset({"message_id", "text"}),
        "delete-message": frozenset({"message_id"}),
        "react": frozenset({"message_id", "reaction"}),
        "send-media": frozenset({"peer_id", "media_id"}),
        "download-media": frozenset({"peer_id", "message_id"}),
    }
)
...
_OPERATIONS = MappingProxyType(
    {
        ...
        CapabilityFamily.MCUB_CONTROL: frozenset(
            {
                "module-list",
                "module-install",
                "module-reload",
...
def _has_ambient_key(value: Any) -> bool:
    ...
    if isinstance(value, Mapping):
        return any(
            key.strip().lower() in forbidden or _has_ambient_key(nested)
            for key, nested in value.items()
        )
...
def _normalize_payload(
    request: CapabilityRequest, grant: CapabilityGrant, resolver: Callable[..., Any]
) -> Mapping[str, Any]:
    ...
    if request.capability is CapabilityFamily.TELEGRAM:
        ...
        if _has_ambient_key(payload["data"]):
            raise CapabilityProtocolError(
                "telegram payload contains ambient credentials"
            )
        ...
        if not required.issubset(payload["data"]):
            raise CapabilityProtocolError(
                "telegram operation is missing required opaque fields"
            )
        for key, value in payload["data"].items():
            if key.endswith("_id") and (not isinstance(value, str) or not value):
                raise CapabilityProtocolError(
                    "telegram references must be opaque non-empty IDs"
                )
        return MappingProxyType({"data": payload["data"]})
    elif request.capability is CapabilityFamily.WORKSPACE_FS:
        allowed = (
            frozenset({"path", "content", "mode", "expected_hash"})
            if request.operation == "write"
            else frozenset({"path"})
        )
        _only(payload, allowed)
        path = (
            _relative_directory(payload.get("path"))
            if request.operation == "list"
            else _relative_path(payload.get("path"))
        )
        ...
        if request.operation == "write":
            ...
            if expected_hash is not None and (
                ...
                raise CapabilityProtocolError(
                    "write expected_hash must be a non-empty string"
                )
        ...
        _only(
            payload,
            frozenset({"argv", "cwd", "timeout_seconds", "max_output_bytes", "env"}),
        )
        ...
        if (
            ...
            raise CapabilityProtocolError(
                "process requests require a non-empty argv array"
            )
        ...
        cwd = resolve_workspace_directory(
            grant.constraints["cwd_root"], payload.get("cwd", ".")
        )
        ...
        if (
            ...
            or timeout <= 0
            or timeout > grant.constraints["max_timeout_seconds"]
        ):
            raise CapabilityProtocolError("process timeout exceeds grant")
        max_output = _positive_bound(
            payload.get("max_output_bytes"),
            "max output",
            grant.constraints["max_output_bytes"],
        )
        if "env" in payload:
            if (
                not isinstance(payload["env"], Mapping)
                or not set(payload["env"]).issubset(
                    set(grant.constraints.get("env_allowlist", ()))
                )
                or any(not isinstance(value, str) for value in payload["env"].values())
            ):
        ...
        return MappingProxyType(
            {
                ...
                "env": payload.get("env", MappingProxyType({})),
            }
        )
        ...
        max_bytes = _positive_bound(
            payload.get("max_bytes"), "HTTPS bytes", grant.constraints["max_bytes"]
        )
        return MappingProxyType(
            {
                "url": url,
        ...
        if child["parent_call_id"] != request.call_id or child[
            "cancellation_parent_id"
        ] != grant.constraints.get("cancellation_parent_id"):
        ...
        for budget in ("remaining_calls", "remaining_token_budget", "remaining_depth"):
            if (
                not isinstance(child[budget], int)
                or child[budget] < 0
                or child[budget] >= grant.constraints.get(budget, -1)
            ):
                raise CapabilityProtocolError("child budget is not strictly reduced")
        canonical_tool_id = normalize_tool_name(
            child["canonical_tool_id"], canonical=True
        )
        ...
        if not isinstance(ancestors, (list, tuple)) or any(
            ...
            raise CapabilityProtocolError(
                "scheduling ancestry must be a string sequence"
            )
        ...
        if not isinstance(ancestor_tools, (list, tuple)) or any(
            not isinstance(item, str) for item in ancestor_tools
        ):
            raise CapabilityProtocolError(
                "scheduling tool ancestry must be a string sequence"
            )
        ...
        if request.operation == "module-install":
            ...
            if (
                ...
                or parsed.username
                or parsed.password
                or len(module_url) > 2_048
            ):
                raise CapabilityProtocolError(
                    "module installation requires a bounded credential-free HTTPS URL"
                )
        ...
        if (
            not isinstance(allowed_keys, (list, tuple, frozenset))
        ...
        if not isinstance(namespace, str) or not (
            ...
            raise CapabilityProtocolError(
                "configuration key lies outside grant namespace"
            )
