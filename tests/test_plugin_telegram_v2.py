...
class FakeTelegramTransport:
    ...
    def request(self, frame: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        return {
            "ok": True,
            "data": {"artifact_id": "artifact-1", "message_id": "message-1"},
        }
...
def test_every_telegram_tool_uses_one_exact_opaque_capability_request(
    module: object,
) -> None:
    for declaration in module.MANIFEST.tools:
        ...
        call = ToolCall(
            call_id=f"call-{declaration.canonical_id}",
            spec=declaration.to_tool_spec(),
            requested_name=declaration.canonical_id,
            arguments=_arguments(declaration.input_schema),
        )
        ...
        assert transport.frames == [
            {
                ...
                "host_request_id": "host-1",
                "call_id": call.call_id,
                "canonical_tool_id": declaration.canonical_id,
                "actor_scope": "actor:test",
                "grant_id": "grant-1",
                "capability": "telegram",
                "operation": result["operation"],
                "capability_request_id": f"{call.call_id}:{declaration.canonical_id}",
...
def test_aliases_and_strict_arguments_follow_the_frozen_matrix() -> None:
    ...
    declaration = next(
        tool
        for tool in message.MANIFEST.tools
        if tool.canonical_id == "message.send_current"
    )
    assert declaration.aliases == ("message.send", "send_message")
    with pytest.raises(Exception):
        ToolCall(
            "bad",
            declaration.to_tool_spec(),
            "message.send",
            {"text": "x", "client": "raw"},
        )
...
def test_moderation_requires_parent_confirmation_before_transport() -> None:
    ...
    declaration = next(
        tool
        for tool in moderation.MANIFEST.tools
        if tool.canonical_id == "moderation.ban"
    )
    call = ToolCall(
        "moderation-call",
        declaration.to_tool_spec(),
        declaration.canonical_id,
        {"peer_id": "peer-1", "user_id": "user-1"},
    )
    entry = next(
        entry
        for entry in TOOL_COMPATIBILITY_MATRIX
        if entry.canonical_id == declaration.canonical_id
    )
    policy = ToolPolicyEngine(
        ToolPolicyCatalog((ToolPolicyRule.from_compatibility(entry),))
    )
    ...
    response = CapabilityBroker(
        policy, {CapabilityFamily.TELEGRAM: transport}
    ).dispatch(
        call,
        ToolPolicyRequest(
            frozenset({declaration.canonical_id}), frozenset({entry.capability_class})
        ),
        grant,
        request,
    )
