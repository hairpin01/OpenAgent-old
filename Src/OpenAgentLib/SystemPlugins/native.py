...
ServiceInvoker = Callable[["NativeSystemToolServices", JsonObject], Awaitable[JsonObject]]
...
_INPUT_SCHEMAS: Mapping[str, JsonObject] = {
    ...
    "code.generate_file": _object({"path": _STRING, "content": _STRING}, ("path", "content")),
    "code.generate_mcub_module": _object({"name": _STRING, "content": _STRING}, ("name", "content")),
    ...
    "context.tool_output": _object({"path": _STRING, "latest": _BOOLEAN, "mode": {"type": "string", "enum": ["head", "tail", "all"]}, "limit": _INTEGER, "offset": _INTEGER}),
    ...
    "skills.import_md": _object({"name": _STRING, "content": _STRING}, ("name", "content")),
    ...
    "skills.save_from_ai": _object({"name": _STRING, "content": _STRING}, ("name", "content")),
    ...
    "todo.edit": _object({"id": _STRING, "text": _STRING, "status": {"type": "string", "enum": ["open", "closed"]}}, ("id", "text")),
...
async def _invoke(services: NativeSystemToolServices, method: ServiceMethod, arguments: JsonObject) -> JsonObject:
    return await method(arguments)


def _service_handler(method: Callable[[NativeSystemToolServices], ServiceMethod]) -> ServiceInvoker:
    async def invoke(services: NativeSystemToolServices, arguments: JsonObject) -> JsonObject:
...
_INVOKERS: Mapping[str, ServiceInvoker] = {
    ...
    "code.choose_filename": _service_handler(lambda service: service.code_choose_filename),
    "code.generate_file": _service_handler(lambda service: service.code_generate_file),
    "code.generate_mcub_module": _service_handler(lambda service: service.code_generate_mcub_module),
    ...
    "context.media_context": _service_handler(lambda service: service.context_media_context),
    ...
    "context.reply_context": _service_handler(lambda service: service.context_reply_context),
    "context.tool_output": _service_handler(lambda service: service.context_tool_output),
    ...
    "skills.save_from_ai": _service_handler(lambda service: service.skills_save_from_ai),
    ...
    "utility.placeholders": _service_handler(lambda service: service.utility_placeholders),
    "utility.plugin_docs": _service_handler(lambda service: service.utility_plugin_docs),
    "utility.random_template": _service_handler(lambda service: service.utility_random_template),
    "utility.search_tool": _service_handler(lambda service: service.utility_search_tool),
    "utility.token_usage": _service_handler(lambda service: service.utility_token_usage),
...
def _system_entries() -> Mapping[str, ToolCompatibility]:
    ...
    if set(entries) != set(_INPUT_SCHEMAS) or set(entries) != set(_INVOKERS):
        raise RuntimeError("native system definitions must cover the frozen system matrix exactly")
    return entries
...
def build_native_system_tools(services: NativeSystemToolServices) -> NativeSystemTools:
    ...
    for canonical_id, entry in entries.items():
        ...
        async def handler(call: ToolCall, invoke: ServiceInvoker = invoker) -> JsonObject:
