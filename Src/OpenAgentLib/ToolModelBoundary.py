    ...
    def __post_init__(self) -> None:
        for field_name, item_type in (
            ...
            if not isinstance(values, tuple) or any(
                ...
                raise TypeError(
                    f"{field_name} must be an immutable tuple of {item_type.__name__}"
                )
    ...
    def __post_init__(self) -> None:
        ...
        for name, cap in caps.items():
            ...
            if (
                ...
                raise ValueError(
                    f"{name} must be a positive integer no larger than {cap}"
                )
...
class ToolModelBoundary:
        ...
        if blocks:
            ...
            for format_, offset, body, recipient in blocks:
                ...
                if isinstance(parsed_or_error, ModelToolError):
                    return ModelBoundaryOutput(
                        ModelTurnKind.INVALID, errors=(parsed_or_error,)
                    )
            ...
            calls_or_error = self._create_calls(
                tuple(sorted(parsed, key=lambda item: item.offset))
            )
            if isinstance(calls_or_error, ModelToolError):
                return ModelBoundaryOutput(
                    ModelTurnKind.INVALID, errors=(calls_or_error,)
                )
            return ModelBoundaryOutput(
                ModelTurnKind.TOOLS, calls_or_error, tuple(parsed)
            )
        final = self.explicit_final(text)
        if final is not None:
            return ModelBoundaryOutput(ModelTurnKind.FINAL, final_answer=final)
        return ModelBoundaryOutput(
            ModelTurnKind.INTERMEDIATE, intermediate=self._intermediate(text)
        )
    ...
    def search_tool_docs(
        ...
        maximum = min(
            self._limits.max_docs_results,
            max(1, limit or self._limits.max_docs_results),
        )
    ...
    def _blocks(
        self, text: str
    ) -> list[tuple[ModelToolFormat, int, str, str | None]] | ModelToolError:
        ...
        for marker in re.finditer(r"```json\b", text, re.I):
            ...
            if (
                _RAW_CALL_SHAPE_RE.search(segment)
                and marker.start() not in valid_fence_starts
            ):
                return self._error(
                    ModelToolErrorCode.MALFORMED_BLOCK,
                    marker.start(),
        ...
        for match in _HARMONY_RE.finditer(text):
            ...
            if recipient is None:
                return self._error(
                    ModelToolErrorCode.MALFORMED_BLOCK,
                    match.start(),
                    ModelToolFormat.HARMONY,
                )
            blocks.append(
                (ModelToolFormat.HARMONY, match.start(), match.group("body"), recipient)
            )
            harmony_spans.append((match.start(), match.end()))
        if harmony_intent and not harmony_spans:
            return self._error(
                ModelToolErrorCode.MALFORMED_BLOCK, 0, ModelToolFormat.HARMONY
            )
        ...
        for match in _XML_RE.finditer(text):
            ...
            if name in {"final", "final_answer"} or any(
        ...
        for marker in _XML_MARKER_RE.finditer(text):
            ...
            if name in {"final", "final_answer"} or any(
                start <= marker.start() < end for start, end in occupied
            ):
                continue
        ...
        has_raw_block = any(
            format_ is ModelToolFormat.RAW_JSON
            for format_, _offset, _body, _recipient in blocks
        )
        if raw_shape and not (
            has_raw_block
            or any(start <= raw_shape.start() < end for start, end in occupied)
        ):
            return self._error(
                ModelToolErrorCode.MALFORMED_BLOCK,
                raw_shape.start(),
                ModelToolFormat.RAW_JSON,
            )
    ...
    def _parse_block(
        self, format_: ModelToolFormat, offset: int, body: str, recipient: str | None
    ) -> tuple[ParsedToolCall, ...] | ModelToolError:
    ...
    def _parse_json_item(
        ...
        offset: int,
        format_: ModelToolFormat,
        recipient: str | None,
    ) -> ParsedToolCall | ModelToolError:
        allowed = {
            "tool",
            "name",
            "args",
        ...
        if "body" in item:
            converted = self._add_body(
                str(name), dict(args), item["body"], offset, format_
            )
        ...
        return ParsedToolCall(
            str(name),
            args,
            self._optional_text(item.get("api_version")),
            self._optional_text(item.get("schema_version")),
            offset,
            format_,
        )

    def _parse_xml(
        self, offset: int, name: str, raw_attrs: str, body: str
    ) -> tuple[ParsedToolCall, ...] | ModelToolError:
        ...
        while cursor < len(raw_attrs):
            ...
            if match is None:
                if raw_attrs[cursor:].strip():
                    return self._error(
                        ModelToolErrorCode.MALFORMED_ATTRIBUTES,
                        offset + cursor,
                        ModelToolFormat.XML,
                    )
                break
            ...
            if key in attrs:
                return self._error(
                    ModelToolErrorCode.DUPLICATE_CALL_ID,
                    offset + cursor,
                    ModelToolFormat.XML,
                )
        ...
        if name == "tool_call":
            ...
            if len(names) != 1:
                return self._error(
                    ModelToolErrorCode.AMBIGUOUS_PAYLOAD, offset, ModelToolFormat.XML
                )
            name = names[0]
    ...
    def _convert_xml_args(
        self, name: str, attrs: Mapping[str, str], body: str, offset: int
    ) -> Mapping[str, Any] | ModelToolError:
        try:
            spec = self._registry.resolve(name)
        except ToolKernelError:
            return self._error(
                ModelToolErrorCode.UNKNOWN_TOOL, offset, ModelToolFormat.XML
            )
        properties = spec.input_schema.get("properties", {})
        if any(key not in properties for key in attrs):
            return self._error(
                ModelToolErrorCode.UNKNOWN_FIELD, offset, ModelToolFormat.XML
            )
        ...
        for key, value in attrs.items():
            ...
            if converted is _INVALID:
                return self._error(
                    ModelToolErrorCode.INVALID_ARGUMENT, offset, ModelToolFormat.XML
                )
    ...
    def _add_body(
        ...
        candidates = [
            ...
            if field in properties and field not in arguments
        ]
        if len(candidates) != 1:
            return self._error(ModelToolErrorCode.AMBIGUOUS_PAYLOAD, offset, format_)
    ...
    def _create_calls(
        self, parsed: Sequence[ParsedToolCall]
    ) -> tuple[ToolCall, ...] | ModelToolError:
        calls: list[ToolCall] = []
        seen: set[str] = set()
        for item in parsed:
            try:
                ...
                return self._error(
                    ModelToolErrorCode.UNKNOWN_TOOL, item.offset, item.format
                )
            try:
                ...
                if (
                    ...
                    return self._error(
                        ModelToolErrorCode.DUPLICATE_CALL_ID, item.offset, item.format
                    )
                seen.add(call_id)
                calls.append(
                    self._registry.create_call(
                        call_id=call_id,
                ...
                code = (
                    ModelToolErrorCode.VERSION_MISMATCH
                    if "version" in str(getattr(error, "code", ""))
                    else ModelToolErrorCode.INVALID_ARGUMENT
                )
    ...
    def _depth(value: Any) -> int:
        if isinstance(value, Mapping):
            return 1 + max(
                (ToolModelBoundary._depth(item) for item in value.values()), default=0
            )
        if isinstance(value, list):
            return 1 + max(
                (ToolModelBoundary._depth(item) for item in value), default=0
            )
        return 0
    ...
    def _coerce_legacy(value: str, schema: Mapping[str, Any]) -> Any:
        ...
        if type_ in {"integer", "number", "array", "object"}:
            try:
                ...
                return _INVALID
            if type_ == "integer" and (
                isinstance(parsed, bool) or not isinstance(parsed, int)
            ):
                return _INVALID
            if type_ == "number" and (
    ...
    def _json_value(value: Any) -> Any:
        if isinstance(value, Mapping):
            return {
                str(key): ToolModelBoundary._json_value(item)
        ...
        if isinstance(value, Mapping):
            return {
                str(key): (
                    ...
                    else ToolModelBoundary._redact(item)
                )
                for key, item in value.items()
            }
        if isinstance(value, list):
    ...
    @staticmethod
    def _error(
        code: ModelToolErrorCode, offset: int, format_: ModelToolFormat | None
    ) -> ModelToolError:
        return ModelToolError(code, offset, format_)

    def _invalid(
        self, code: ModelToolErrorCode, offset: int, format_: ModelToolFormat | None
    ) -> ModelBoundaryOutput:
        return ModelBoundaryOutput(
            ModelTurnKind.INVALID, errors=(self._error(code, offset, format_),)
        )
