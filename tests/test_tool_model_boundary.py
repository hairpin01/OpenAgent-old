...
def test_legacy_forms_normalize_to_registry_tool_calls(
    ...
    for value in values:
        ...
        assert output.calls[0].arguments == {
            "command": "ls",
            "count": 2,
            "enabled": True,
            "tags": ("a",),
...
def test_xml_body_uses_only_declared_body_compatibility_field(
    tool_registry_builder, tool_spec_builder
) -> None:
    boundary = _boundary(tool_registry_builder, tool_spec_builder)
...
def test_malformed_ambiguous_and_unknown_calls_never_produce_calls(
    tool_registry_builder, tool_spec_builder
) -> None:
    boundary = _boundary(tool_registry_builder, tool_spec_builder)
    cases = (
        ...
        (
            '{"tool":"run","args":{"command":"ls","extra":1}}',
            ModelToolErrorCode.INVALID_ARGUMENT,
        ),
        (
            '{"tool":"run","args":{"command":"ls"},"id":"model-id"}',
            ModelToolErrorCode.UNKNOWN_FIELD,
        ),
...
def test_rejects_oversized_deep_and_duplicate_factory_ids(
    tool_registry_builder, tool_spec_builder
) -> None:
    ...
    assert (
        deep.parse('{"tool":"run","args":{"command":"ls","tags":[["x"]]}}')
        .errors[0]
        .code
        is ModelToolErrorCode.NESTING_TOO_DEEP
    )
    ...
    duplicate = ToolModelBoundary(
        tool_registry_builder((spec,)),
        call_id_factory=lambda: "same",
        context=ToolContext("c"),
    )
    result = duplicate.parse(
        '```tool_call\n[{"tool":"run","args":{"command":"a"}},{"tool":"run","args":{"command":"b"}}]\n```'
    )
    assert result.errors[0].code is ModelToolErrorCode.DUPLICATE_CALL_ID


def test_legacy_scalar_coercion_rejects_boolean_as_integer(
    tool_registry_builder, tool_spec_builder
) -> None:
    boundary = _boundary(tool_registry_builder, tool_spec_builder)
...
def test_explicit_final_is_not_accepted_when_tool_content_is_present(
    tool_registry_builder, tool_spec_builder
) -> None:
    boundary = _boundary(tool_registry_builder, tool_spec_builder)
...
def test_docs_and_search_are_deterministic_and_do_not_expose_source_details(
    tool_registry_builder, tool_spec_builder
) -> None:
    ...
    boundary = ToolModelBoundary(
        tool_registry_builder((second, first)),
        call_id_factory=lambda: "id",
        context=ToolContext("c"),
    )
    docs = boundary.tool_docs()
    assert [doc["id"] for doc in docs] == ["alpha.read", "zeta.read"]
    assert "source_module" not in docs[0]
    assert [doc["id"] for doc in boundary.search_tool_docs("read", limit=1)] == [
        "alpha.read"
    ]


def test_result_rendering_redacts_errors_and_spills(
    tool_registry_builder, tool_spec_builder
) -> None:
    boundary = _boundary(
    ...
    result = ToolResult(
        ...
        output={"token": "secret", "nested": {"password": "hidden"}},
    )
    ...
    spilled = json.loads(
        boundary.render_result(
            large, spill_reference=lambda _result, _text: "spill://result/2"
        )
    )
    assert spilled == {
        "call_id": "call-2",
        "spill_ref": "spill://result/2",
        "status": "success",
    }
...
def test_malformed_tool_markers_never_fall_back_to_prose(
    tool_registry_builder, tool_spec_builder
) -> None:
    boundary = _boundary(tool_registry_builder, tool_spec_builder)
    malformed = (
...
def test_non_tool_prose_and_json_examples_remain_intermediate(
    tool_registry_builder, tool_spec_builder
) -> None:
    boundary = _boundary(tool_registry_builder, tool_spec_builder)
    values = (
