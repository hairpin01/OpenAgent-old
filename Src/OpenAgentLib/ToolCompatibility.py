...
def _system_declarations(root: Path) -> tuple[dict[str, Any], ...]:
    ...
    for path in sorted(root.rglob("*.py")):
        ...
        keywords = {
            keyword.arg: literal_eval(keyword.value) for keyword in descriptor.keywords
        }
        ...
        declarations.append(
            {
                ...
                "aliases": tuple(
                    _normalized(alias) for alias in keywords.get("aliases", ())
                ),
                ...
                "source_module": path.relative_to(root)
                .with_suffix("")
                .as_posix()
                .replace("/", "."),
            }
        ...
        if len(classes) != 1:
            ...
            if not has_v2_manifest:
                raise CompatibilityInventoryError(
                    f"expected one plugin class in {path}"
                )
            entries = tuple(
                entry
                for entry in TOOL_COMPATIBILITY_MATRIX
                if entry.source_module == path.stem
            )
            if not entries:
                raise CompatibilityInventoryError(
                    f"v2 plugin {path} has no frozen compatibility entries"
                )
            tool_map = {
            ...
            declarations.append(
                {
                    ...
                    "dangerous": frozenset(
                        entry.canonical_id
                        for entry in entries
                        if entry.confirmation_class == "required"
                    ),
        ...
        declarations.append(
            {
                ...
                "dangerous": {
                    _normalized(tool)
                    for tool in _literal_assignment(body, "dangerous_tools", set())
                },
                "source_module": path.stem,
            }
        )
...
def _plugin_aliases(
    canonical_ids: tuple[str, ...], tool_map: Mapping[str, str]
...
def _classification(
    canonical_id: str, source_family: str, dangerous: bool, parallel_safe: bool
    ...
    mutating = dangerous or capability not in {
        "read-only",
        "filesystem-read",
        "telegram-read",
    }
    ...
    concurrency = (
        "parallel-read"
        if source_family == "system" and parallel_safe and not mutating
        else "serial"
    )
...
def _entry(
    ...
    v2_input = _readonly_mapping(
        input_schema or {"status": "placeholder", "type": "object"}
    )
    v2_output = _readonly_mapping(
        output_schema or {"status": "placeholder", "legacy_result": "text"}
    )
    ...
    for declaration in _system_declarations(system_root):
        entries.append(
            _entry(
                ...
                declaration["aliases"],
                "system",
                declaration["source_module"],
                declaration["docs"],
                declaration["dangerous"],
                declaration["parallel_safe"],
        ...
        for canonical_id in declaration["canonical_ids"]:
            entries.append(
                _entry(
                    ...
                    "sibling-plugin",
                    declaration["source_module"],
                    declaration["docs"].get(canonical_id, {}),
                    canonical_id in declaration["dangerous"],
                    False,
                )
            )
...
def validate_compatibility_matrix(matrix: Iterable[ToolCompatibility]) -> None:
    ...
    for entry in matrix:
        ...
        if canonical_id in canonical_ids:
            raise CompatibilityInventoryError(
                f"duplicate canonical ID {canonical_id!r}"
            )
        if any(not _normalized(getattr(entry, field)) for field in required):
            raise CompatibilityInventoryError(
                f"unclassified canonical ID {canonical_id!r}"
            )
        ...
        for name in (canonical_id, *entry.aliases):
            ...
            if not normalized_name:
                raise CompatibilityInventoryError(
                    f"empty alias for canonical ID {canonical_id!r}"
                )
...
_TOOL_COMPATIBILITY_SNAPSHOT = (
    (
        ...
        (
            (
                "attrs",
                "pattern (str); rewrite/replace (str); lang/language (str); path/paths (str); glob/globs (str); apply/update (bool, default false)",
            ),
            ("body", "optional 'pattern -> rewrite' format when attrs are omitted"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "filesystem-write",
        "serial",
        "non-idempotent",
    ...
    (
        ...
        (
            (
                "attrs",
                "pattern (str); lang/language (str); path/paths (str, default '.'); glob/globs (str); json (bool|string)",
            ),
    ...
    (
        ...
        "sibling-plugin",
        "chat",
        (("attrs", "chat (str) — chat identifier"), ("body", "chat identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "sibling-plugin",
        "chat",
        (("attrs", "user (str) — target user"), ("body", "user identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "chat",
        (
            ("attrs", "chat (str) or query (str) — username, ID, or link"),
    ...
    (
        ...
        "sibling-plugin",
        "chat",
        (("attrs", "chat (str) — target chat"), ("body", "not used")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "chat",
        (
            ("attrs", "chat (str); limit (int) — max results (default 30)"),
            ("body", "chat identifier"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        (
            (
                "attrs",
                "chat (str) — target; about (str) or description (str) — new text",
            ),
            ("body", "about text"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        "chat",
        (
            ("attrs", "chat (str) — target; title (str) — new title"),
            ("body", "title text"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "chat",
        (
            ("attrs", "chat (str) — target; username (str) — new username"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "chat",
        (
            ("attrs", "chat (str) — target; seconds (int) — slowmode delay"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "system",
        "Code.choose_filename",
        (("attrs", "name/path"), ("body", "optional filename")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "read-only",
    ...
    (
        ...
        "system",
        "Code.generate_file",
        (("attrs", "name/path"), ("body", "file content")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "filesystem-write",
    ...
    (
        ...
        "contacts",
        (
            ("attrs", "user (str); first_name (str); last_name (str); phone (str)"),
    ...
    (
        ...
        "sibling-plugin",
        "contacts",
        (("attrs", "user (str) — user to block"), ("body", "user identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "sibling-plugin",
        "contacts",
        (("attrs", "user (str) — user to remove"), ("body", "user identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "sibling-plugin",
        "contacts",
        (("attrs", "user (str) — username or ID"), ("body", "user identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "sibling-plugin",
        "contacts",
        (("attrs", "user (str) — user to unblock"), ("body", "user identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "system",
        "Context.discard",
        (("attrs", "target/all; keep"), ("body", "optional target list")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "state-write",
    ...
    (
        "context.tool_output",
        ("context.read_tool_output", "tool_output.read"),
        "system",
        "Context.tool_output",
        (
            ("attrs", "path/file/id; latest=true; mode=head|tail|all; limit; offset"),
    ...
    (
        ...
        (
            (
                "attrs",
                "name (str) or title (str); username (str) or bot (str); about (str)",
            ),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        "creation",
        (
            ("attrs", "title (str) or name (str); about (str) or description (str)"),
            ("body", "title"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "sibling-plugin",
        "creation",
        (("attrs", "link (str) — invite link"), ("body", "invite link")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "sibling-plugin",
        "dialog",
        (("attrs", "chat (str) or id (str) — target"), ("body", "chat identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "sibling-plugin",
        "dialog",
        (("attrs", "chat (str) — target"), ("body", "not used")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "sibling-plugin",
        "dialog",
        (("attrs", "q (str) or query (str) — search term"), ("body", "search term")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "dialog",
        (
            ("attrs", "chat (str) — target; photo (str) — file path"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "eval",
        (
            ("attrs", "code (str) or expr (str); timeout (int)"),
            (
                "body",
                "Python code. You may use await and return from the async function.",
            ),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        "file.download_media",
        ("download_media", "file.download"),
        "sibling-plugin",
        "file",
        (
            (
                "attrs",
                "message (str) or msg (str) — message ID; chat (str) or from (str)",
            ),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        "file",
        (
            ("attrs", "path (str); search (str); replace (str); count (int)"),
    ...
    (
        ...
        "file",
        (
            ("attrs", "path (str); reverse (str) — 'true' to reverse"),
    ...
    (
        ...
        "sibling-plugin",
        "file",
        (("attrs", "path (str) or file (str) or name (str)"), ("body", "file path")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "filesystem-read",
    ...
    (
        ...
        "file",
        (
            ("attrs", "path (str) or file (str); chat (str) — target; caption (str)"),
            ("body", "file path"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "file",
        (
            ("attrs", "path (str) — file; mode (str) — 'overwrite' or 'append'"),
    ...
    (
        ...
        "mcub",
        (
            ("attrs", "command (str) or cmd (str) — command text (prefix auto-added)"),
            ("body", "command text"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        (
            (
                "attrs",
                "command (str) or query (str) — config command like 'module.key=value'",
            ),
            ("body", "config command"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        "mcub",
        (
            ("attrs", "command (str) or query (str) — module URL or name"),
            ("body", "URL or name"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "sibling-plugin",
        "mcub",
        (("attrs", "command (str) or query (str)"), ("body", "not used")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "runtime-control",
    ...
    (
        ...
        "sibling-plugin",
        "message",
        (("attrs", "ids (str) — comma-separated IDs"), ("body", "not used")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "sibling-plugin",
        "message",
        (("attrs", "chat (str); message (str) — draft text"), ("body", "draft text")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "message",
        (
            ("attrs", "message (str) or msg (str) — message ID or link"),
            ("body", "new text"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "message",
        (
            ("attrs", "from_chat (str); msg_id (int); to (str) — destination"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "message",
        (
            ("attrs", "chat (str); message (str) or msg (str) — message ID"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "message",
        (
            ("attrs", "chat (str) — optional; limit (int) — max messages (default 20)"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "sibling-plugin",
        "message",
        (("attrs", "chat (str); max_id (int) — optional"), ("body", "not used")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "message",
        (
            ("attrs", "chat (str) — target; message (str) or msg (str) — message ID"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        (
            (
                "attrs",
                "chat (str); message (str) or msg (str) — message ID; emoji (str) — reaction",
            ),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        (
            (
                "attrs",
                "chat (str); reply_to (int) — message ID; message (str) — content",
            ),
            ("body", "reply text"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        (
            (
                "attrs",
                "chat (str); message (str) — content; schedule (int) — unix timestamp",
            ),
            ("body", "message text"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        (
            (
                "attrs",
                "q (str) or query (str) — search text; chat (str) — optional scope",
            ),
            ("body", "query text"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        "sibling-plugin",
        "message",
        (("attrs", "message (str) or text (str) — content"), ("body", "message text")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        (
            (
                "attrs",
                "chat (str) or to (str) — target; message (str) or text (str) — content",
            ),
            ("body", "message text"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        "moderation",
        (
            ("attrs", "chat (str); user (str); reason (str) — optional"),
            ("body", "user identifier"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "moderation",
        (
            ("attrs", "chat (str); ids (str) — comma-separated IDs"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        "moderation.demote",
        ("chat.demote", "demote_user"),
        "sibling-plugin",
        "moderation",
        (("attrs", "chat (str); user (str)"), ("body", "user identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        "moderation.get_admins",
        (),
        "sibling-plugin",
        "moderation",
        (("attrs", "chat (str)"), ("body", "not used")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "moderation",
        (
            ("attrs", "chat (str); user (str); until (int) — seconds (default 3600)"),
            ("body", "user identifier"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        "moderation.promote",
        ("chat.promote", "promote_user"),
        "sibling-plugin",
        "moderation",
        (
            ("attrs", "chat (str); user (str); rank (str) — admin title"),
            ("body", "user identifier"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "sibling-plugin",
        "profile",
        (("attrs", "target (str) or user (str)"), ("body", "user identifier")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "telegram-read",
    ...
    (
        ...
        "profile",
        (
            ("attrs", "target (str) or user (str) — username or ID"),
            ("body", "user identifier"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        (
            (
                "attrs",
                "target (str) or user (str); download (str) — set to 'true' to download",
            ),
            ("body", "user identifier"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        "sibling-plugin",
        "profile",
        (("attrs", "photo (str) or file (str) — path to image"), ("body", "not used")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "sibling-plugin",
        "profile",
        (("attrs", "bio (str) or about (str) — new bio text"), ("body", "bio text")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "profile",
        (
            ("attrs", "first_name (str); last_name (str) — optional"),
            ("body", "not used"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "sibling-plugin",
        "profile",
        (("attrs", "username (str) — new username (without @)"), ("body", "not used")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "telegram-admin",
    ...
    (
        ...
        "system",
        "SkillsAgent.save",
        (("attrs", "name/title"), ("body", "skill markdown/content")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "filesystem-write",
    ...
    (
        ...
        (),
        "system",
        "SkillsAgent.activate",
        (("attrs", "query/name"), ("body", "optional query")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "read-only",
    ...
    (
        ...
        (),
        "system",
        "SkillsAgent.export_md",
        (("attrs", "name"), ("body", "optional skill name")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "read-only",
    ...
    (
        ...
        (),
        "system",
        "SkillsAgent.import_md",
        (("attrs", "name/title"), ("body", "markdown content")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "filesystem-write",
    ...
    (
        ...
        (),
        "system",
        "SkillsAgent.save_from_ai",
        (("attrs", "name/title"), ("body", "skill content")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "filesystem-write",
    ...
    (
        ...
        (
            (
                "attrs",
                "tool/name (str) — tool to run; attrs (str) — inner attrs; label (str)",
            ),
    ...
    (
        ...
        "task",
        (
            ("attrs", "tool/name (str); attrs (str); label (str)"),
            ("body", "body for the inner tool"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "terminal",
        (
            ("attrs", "command (str) or cmd (str) — command to run"),
            ("body", "command text"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "sibling-plugin",
        "terminal",
        (("attrs", "path (str) — directory path (default: .)"), ("body", "path text")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "filesystem-read",
    ...
    (
        ...
        "sibling-plugin",
        "terminal",
        (("attrs", "path (str) or file (str) — file to read"), ("body", "path text")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "filesystem-read",
    ...
    (
        ...
        "terminal",
        (
            ("attrs", "command (str) or cmd (str) — shell command to execute"),
            ("body", "command text"),
        ),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
    ...
    (
        ...
        "system",
        "Thinking.note",
        (("attrs", "note/text"), ("body", "optional note text")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "state-write",
    ...
    (
        ...
        "system",
        "Todo.edit",
        (("attrs", "id/index/text/status"), ("body", "")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "state-write",
    ...
    (
        ...
        "Utility.plugin_docs",
        (
            ("attrs", "plugin/name (str, optional) — plugin to inspect"),
    ...
    (
        ...
        "Utility.search_tool",
        (
            ("attrs", "query (str) — capability or tool name to search for"),
    ...
    (
        ...
        "system",
        "Utility.tool_help",
        (("attrs", "tool (str) — exact tool name"), ("body", "optional tool name")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "none",
        "read-only",
    ...
    (
        ...
        "sibling-plugin",
        "web",
        (("attrs", "url (str) — page to scan"), ("body", "URL")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "network",
    ...
    (
        ...
        "sibling-plugin",
        "web",
        (("attrs", "url (str) — the web page to fetch"), ("body", "URL")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "network",
    ...
    (
        ...
        "sibling-plugin",
        "web",
        (("attrs", "url (str) — page to read"), ("body", "URL")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "network",
    ...
    (
        ...
        (
            (
                "attrs",
                "query (str) or q (str) or url (str) — search query or web page URL",
            ),
            ("body", "query or URL"),
        ),
        (("status", "placeholder"), ("type", "object")),
    ...
    (
        ...
        "sibling-plugin",
        "web",
        (("attrs", "url (str) — page to summarize"), ("body", "URL")),
        (("status", "placeholder"), ("type", "object")),
        (("status", "placeholder"), ("legacy_result", "text")),
        "required",
        "network",
